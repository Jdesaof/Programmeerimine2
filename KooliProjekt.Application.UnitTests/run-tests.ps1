...
$displayFiles = @($files | Where-Object { $_.IsDelete -or $_.File -match '(^|[\\/])Save\w+Command(Handler|Validator)\.cs$' })
...
if([int]$counters.passed -lt 417){throw "Expected at least 417 passed tests, got $($counters.passed)."}
