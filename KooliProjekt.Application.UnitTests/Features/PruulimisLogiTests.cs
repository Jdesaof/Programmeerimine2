...
namespace KooliProjekt.Application.UnitTests.Features
{
    public class PruulimisLogiTests : ServiceTestBase
    {
        ...
        public async Task Search_should_apply_individual_and_combined_filters(string text, int? second, string expected)
        {
            ...
            var handler = new ListPruulimisLogidQueryHandler(DbContext);
            var result = await handler.Handle(new ListPruulimisLogidQuery
            {
                Page = 1, PageSize = 100, Kasutaja = text, PartiiId = second
            }, CancellationToken.None);
        ...
        public async Task Search_should_filter_before_paging(int page, int expectedId)
        {
            ...
            var result = await handler.Handle(new ListPruulimisLogidQuery
            {
                Page = page, PageSize = 1, Kasutaja = "Alpha", PartiiId = 7
            }, CancellationToken.None);
            Assert.False(result.HasErrors);
        ...
        private async Task SeedSearchRecords()
        {
            foreach (var id in new[] { 4, 3, 2, 1 })
            {
                ...
                entity.Kasutaja = id == 2 ? "Beta" : "Alpha " + id;
                entity.PartiiId = id == 3 ? 8 : 7;
                DbContext.PruulimisLogid.Add(entity);
            }
            await DbContext.SaveChangesAsync();
            DbContext.ChangeTracker.Clear();
        }
